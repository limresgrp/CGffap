...
class ForceModel(torch.nn.Module):
   ...
   def dihedral(self, bead_pos):
      ...
      torsion = torch.arccos(torch.sum(binorm1*binorm2, dim=-1)) - torch.pi
   ...
   def forward(self, positions: torch.Tensor):
         ...
         lj_energy = 4 * torch.multiply(torch.abs(self.dispertion_const[self.nonbond_dict_index]),(torch.pow(torch.div(self.lj_const[self.nonbond_dict_index], norm), 12) - torch.pow(torch.div(self.lj_const[self.nonbond_dict_index], norm), 6)))

         Hbond_lj_energy = torch.multiply(torch.abs(self.bond_H_strength_const[self.nonbond_dict_index]),(torch.pow(torch.div(self.bond_H_lj_const[self.nonbond_dict_index], norm), 6) - torch.pow(torch.div(self.bond_H_lj_const[self.nonbond_dict_index], norm), 4)))
        ...
        #  force_field = dict(zip(self.nonbonded_keys, self.dispertion_const))
         ...
         energies = torch.sum(bond_energy)  + torch.sum(coulumb_energy) + torch.sum(angle_energy) + torch.sum(dihedral_energy)  + torch.sum(proper_dih_energy_BB) #++ torch.sum(lj_energy) torch.sum(Hbond_lj_energy) #+ torch.sum(proper_dih_energy)  # + torch.sum(phantom_energy)  
